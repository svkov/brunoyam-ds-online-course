...
В этом модуле мы будем говорить о хороших практиках в DS и о том как сделать ваше исследование качественным. Все подходы и инструменты, описанные в этом модуле, дополнительные по своей сути, но если вы их освоите, то ваша ценность как специалиста вырастет в разы. Все технологии, описанные в этом модуле, появились только для того, чтобы решать проблемы, с которыми сталкиваются исследователи по всему миру.
...
Второй способ более надежен и лучше использовать его. В крупных компаниях есть специальные средства разработки, которые автоматически оборачивают ваш код веб-приложением, и позволяют не задумываться над интеграциями, в компаниях поменьше приходится это делать самостоятельно. Проще всего использовать такие инструменты как `FastAPI`, `Dash` и `Flask`.
...
защищать результаты, обновлять результаты, если обнаружены ошибки, воспроизводить результаты при обновлении данных, представить свои результаты на обсуждение. Переносите код из jupyter ноутбуков в скрипты, так надежнее.

### Правило №2 - избегайте этапов ручного управления данными или процессом

Может возникнуть соблазн открыть файлы данных в Excel и вручную исправить пару ошибок форматирования или удалить выбросы. Однако соблазну сократить ваш алгоритм следует сопротивляться. Ручная обработка данных - это скрытая манипуляция.

### Правило №3 - сохраните точные версии всех использованных внешних инструментов

В идеале вы должны настроить виртуальную машину или контейнер со всем программным обеспечением, используемым для запуска ваших скриптов. Это позволяет сделать снимок вашей аналитической экосистемы, что упрощает воспроизведение ваших результатов. По крайней мере, вам необходимо задокументировать выпуск и версию всего используемого программного обеспечения, включая операционную систему. Незначительные изменения в программном обеспечении могут повлиять на результаты. Сохраните точные версии библиотек в виртуальном окружении, настройте Docker.
...
Спустя какое-то время вам возможно нужно будет перестроить график по тем же данным. Сохранение этих данных сильно упростит работу в будущем.
...
Однако резюмирование также является простым способом неправильного использования данных, поэтому важно, чтобы ваши коллеги могли разбить сводку на отдельные точки данных. Для каждого итогового результата укажите ссылку на данные, использованные для расчета итогового значения. Проще всего это сделать, если все результаты исследования будут генерироваться скриптами, по которым можно будет отследить откуда получился каждый результат.
