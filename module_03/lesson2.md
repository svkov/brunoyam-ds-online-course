...
Здесь придется идти по всему массиву данных и считать разницу между двумя соседними элементами, обращаясь к ним по индексу. Разницу можно посчитать так: `observations[i] - observations[i - 1]`, однако нужно помнить, что на нулевом шаге мы выйдем за границы массива (нужно начать итерироваться с 1):
...
Если нужно конвертировать кортеж к списку, то это делается при помощи функции `tuple`:
...
Имейте ввиду, что при выполнении этой операции, произойдет копирование данных. поэтому если вы захотите так добавить элемент к кортежу (конвертировать в список, добавить элемент и конвертировать в кортеж), то это будет неоптимально.
...
Обратите внимание, что порядок может не совпадать с тем, в каком порядке вы задавали элементы словаря выше. Элементы словаря хранятся в произвольном порядке.
