...
Давайте прочитаем файл с погодой `weather.csv` при помощи функции `pd.read_csv`. Аргументов в функцию нужно передать путь до файла. В примере ниже предполагается, что файл лежит в папке data.
